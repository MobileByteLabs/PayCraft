...
export interface WebhookEventView {
  ...
  /** Not recorded by webhook_logs. Present so the sections that used to invent them can say so. */
...
function NotRecorded({ what }: { what: string }) {
  // webhook_logs does not persist this. Saying so beats rendering an empty block that reads as
  // "nothing happened", and beats inventing a plausible value outright.
  return (
    <div className="rounded-lg border border-dashed border-ink-200 px-4 py-6 text-center">
      <p className="text-xs text-ink-500">The {what} is not recorded for this delivery.</p>
    </div>
  )
}

export function EventClient({ event }: { event: WebhookEventView }) {

  const payloadStr = JSON.stringify(event.payload, null, 2)
  const responseStr = event.response ? JSON.stringify(event.response, null, 2) : null
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-7 space-y-6">
          ...
          <section className="bg-white border border-ink-200 rounded-xl overflow-hidden shadow-sm">
            <div className="px-5 py-4 border-b border-ink-100 flex items-center justify-between bg-ink-50/50">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] text-ink-400 font-medium">{event.processing_ms === null || event.processing_ms === undefined ? "not recorded" : `${event.processing_ms}ms`}</span>
                </div>
              </div>
            ...
            <div className="bg-ink-950 p-6 overflow-x-auto">
              <pre className="text-[13px] font-mono leading-relaxed text-ink-200 whitespace-pre">
                {responseStr ? <JsonDisplay data={event.response} /> : <NotRecorded what="handler response" />}
              </pre>
            </div>
          </section>
        </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between bg-ink-50 p-2 rounded border border-ink-100">
                  <code className="text-[12px] text-ink-700 font-mono">{event.webhook_id ?? "not recorded"}</code>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between bg-ink-50 p-2 rounded border border-ink-100">
                  <code className="text-[12px] text-ink-700 font-mono">{event.idempotency_key ?? "not recorded"}</code>
                  <InlineCopyButton text={event.idempotency_key ?? ""} />
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[13px] text-ink-500 font-medium">Duration</span>
                <span className="text-[13px] font-semibold text-ink-900">{event.processing_ms === null || event.processing_ms === undefined ? "not recorded" : `${event.processing_ms}ms`}</span>
              </div>
            </div>
          </section>
