-- 142_readiness_razorpay_plans_mode_scoped.sql
--
-- Judge Razorpay readiness PER MODE, now that 141 gives each mode its own plan-id column.
--
-- 140 could only say "3 plans exist but PayCraft cannot tell which mode they belong to", because
-- `razorpay_plan_id_by_currency` was written by whichever sync ran last. That was the honest answer
-- at the time and it is now obsolete: `razorpay_plan_id_by_currency_test` holds test plans,
-- the original column holds live ones, and `live_plan_ids_verified` records whether a LIVE sync has
-- actually written the live column since the split.
--
-- So readiness stops hedging and states a fact in each direction:
--   test_ready  ← a test plan exists (or test links do, for link-style PSPs)
--   live_ready  ← live links exist, OR live plans exist AND have been verified by a live sync
--
-- The `live_plan_ids_verified` conjunct is the part worth keeping honest about. A row carrying
-- pre-141 plan ids is NOT evidence of live readiness: those ids may have been written by a test
-- sync, which is the whole reason 141 exists. Such a row reports live_ready = false with an action
-- that says to re-sync — one live sync clears it permanently.
